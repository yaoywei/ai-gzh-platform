...
## Phase 4: HTML 生成（v4.1 摸鱼小李接管 + 大姚青主题）

**v4.1 启动版使用摸鱼小李 `gzh-design-skill` + 师傅专属主题"大姚青"做排版**（不内置 HTML 生成器，避免 AGPL 传染）。
...
| 强调 | #00A6A6 青绿 |
| 字体 | PingFang SC + 思源黑体 |
| 排版结构 | 完全继承摸鱼绿（6 Parts 目录 + 卡片化 + 14px / 1.75 行高）|
| 渐变 | 135deg 深海蓝→青绿 |
| **AGPL 风险** | **0**（大姚青只留在摸鱼小李 skill，ai-gzh 不 fork）|

### 使用流程

1. 师傅写完 `article.md`（v4.1 5 段式 + 1500-1800 字）
2. 调摸鱼小李 skill 生成 HTML：
   ```bash
   # 主题写法：用「dayao-qing-clone」即可
   npx gzh-design-skill render --input article.md --theme dayao-qing-clone
   ```
   或在 Claude Code 里说："用大姚青把 article.md 排版"
3. 输出 HTML 复制到公众号后台
4. 备选主题：moyu-green（摸鱼小李默认）/ red-white / graphite-minimal / zen-whitespace / moyu-ticket / olive-journal

详见摸鱼小李 skill：https://github.com/isjiamu/gzh-design-skill
...
| **v4 启动版主题说明（2026-07-19 新增）** | `references/v4-启动版-主题说明.md` |
